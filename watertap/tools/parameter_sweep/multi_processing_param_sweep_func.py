...
class paramActor:
    ...
    def build_model(self):
        ...
        if self.json_string is not None:
            ...
            self.model.state_after_box_solve = modelStateStorage(self.model)
            self.model.state_after_box_solve.restore_from_dict(self.json_string)
            iscale.calculate_scaling_factors(self.model)
            self.model.state_after_box_solve = modelStateStorage(self.model)
            # self.optimize_function(self.model, **self.optimize_kwargs)
    ...
    def step_optimize(
        self, m, solver=None, try_final_first=True, check_termination=False
    ):
        # fsTools.solve(m)
        solver = get_solver()
        results = stepTool.step_optimize(
            ...
            steps=5,
            try_final_first=try_final_first,
            re_steps=1,
        )
        return results
